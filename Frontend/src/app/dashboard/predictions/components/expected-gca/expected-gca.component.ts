...
@Component({
  selector: 'app-expected-gca',
  templateUrl: './expected-gca.component.html',
  styleUrl: './expected-gca.component.css'
})
export class ExpectedGCAComponent {
  ...
  sendForm() {
    ...
    this.jugadoresService.expectedGCA(playerData)
      .subscribe(
        response => {
          this.gca = response['expectedGCA'];
          this.mostrarGCA = this.gca.toString().substring(0, 5);          
